...
contract Polatine is ERC721 {
    ...
    mapping(uint256 => address) private _consumeState;
    ...
    function consumeState(uint256 tokenId) public view virtual override returns (string memory) {
        return _consumeState[tokenId];
    }
    ...
    function initiateConsume(uint256 tokenId) 
        public
    {
        require(ownerOf(tokenId) == _msgSender(), "consume caller is not owner");
        _consumeState[tokenId] = _msgSender();
       
    }

    function cancelConsume(uint256 tokenId) 
        public
    {
        require(ownerOf(tokenId) == _msgSender(), "consume caller is not owner");
        delete _consumeState[tokenId];
    }

    function finalizeConsume(uint256 tokenId) 
        public
    {
        require(_tokenSubcollection[tokenId] == _msgSender(), "caller is not the creator of the collection that the token belongs to");
        require(ownerOf(tokenId) == _consumeState[tokenId] , "The owner of the token has not initiated the consume process");
         _burn(tokenId);
    }
